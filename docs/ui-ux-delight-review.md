...
- 楽観的更新、通信失敗時の復元、浮遊XP、5秒間の取り消し、`aria-live`を実装済み
- Daily Clear／Perfectの固定トースト演出と`prefers-reduced-motion`対応を実装済み
- Lucienをタップしたときの短い応答と画像リアクションを実装済み
- Moodごとの専用ポーズへの切り替えは、追加イラスト制作後に実装する
